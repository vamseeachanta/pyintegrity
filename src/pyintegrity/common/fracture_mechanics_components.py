...
class FractureMechanicsComponents():
    ...
    def get_flaw_growth_for_fatigue_loading(self):
        ...
        bending_factor = self.cfg['loading']['histograms']['from_xlsx'][0]['bending_factor']
        self.bs7910_2013_flaw_growth.get_approximate_flaw_growth_due_to_histograms(self.histograms_df, bending_factor)
        self.bs7910_2013_flaw_growth.get_flaw_growth_due_to_histograms(self.histograms_df, bending_factor)
    ...
    def save_result_tables(self):
        file_name = self.cfg['Analysis']['result_folder'] + self.cfg['Analysis']['file_name'] + '_fad.csv'
        ...
        for location_item in self.cfg.default['settings']['location_array']:
            file_name = self.cfg['Analysis']['result_folder'] + self.cfg['Analysis']['file_name'] + '_unstable_fracture_limits.csv'
            self.bs7910_2013_critical_flaw.result[location_item]['fracture_limit'].to_csv(file_name)

        # Approximate minimum flaw solutions for varying starting flaw
        file_name = self.cfg['Analysis']['result_folder'] + self.cfg['Analysis']['file_name'] + '_approximate_min_initial_flaw.csv'
        self.bs7910_2013_flaw_growth.approximate_flaw_growth_df.to_csv(file_name)

        # Time Marching minimum flaw solution
        file_name = self.cfg['Analysis']['result_folder'] + self.cfg['Analysis']['file_name'] + '_flaw_growth.csv'
        self.bs7910_2013_flaw_growth.flaw_growth_df.to_csv(file_name)

        # Time Marching minimum flaw solution
        file_name = self.cfg['Analysis']['result_folder'] + self.cfg['Analysis']['file_name'] + '_time_marching_min_initial_flaw.csv'
        self.bs7910_2013_flaw_growth.flaw_growth_df.to_csv(file_name)
    ...
    def create_viz_fatigue_assessment_diagrams(self):
        ...
        plt_settings.update({'file_name': self.cfg['Analysis']['result_folder'] + self.cfg['Analysis'][
            'file_name'] + '_' + plt_settings['file_name_extension'] + '.png'})
    ...
    def create_viz_unstable_fracture_limits(self):
        ...
        for location_item in self.cfg.default['settings']['location_array']:
            for orientation_item in self.cfg.default['settings']['orientation_array']:
                plt_settings['label'] = [location_item + ' flaw, ' + orientation_item]
                temp_df = self.bs7910_2013_critical_flaw.result[location_item]['fracture_limit']
                viz_data.from_df_columns(temp_df[temp_df.flaw_orientation == orientation_item],
                                         plt_settings)
        viz_data.add_title_and_axis_labels()
        viz_data.add_legend()
        viz_data.add_x_y_lim_formats()
    ...
    def create_viz_min_init_flaw_by_component(self):
        ...
        for cfg_component_index in range(0, len(self.cfg.loading['histograms']['from_xlsx'][0]['component_index'])):
            ...
            component_label = self.cfg.loading['histograms']['from_xlsx'][0]['component_label'][cfg_component_index]
            plt_settings = self.cfg['plot_settings'][2].copy()
            plt_settings['title'] = plt_settings['title'].format(component_label)
            plt_settings.update({'file_name': self.cfg['Analysis']['result_folder'] + self.cfg['Analysis'][
                'file_name'] + '_' + plt_settings['file_name_extension'] + '_component_' + str(
                component_index) + '.png'})
            ...
            for location_item in self.cfg.default['settings']['location_array']:
                for service_life in self.cfg.default['settings']['service_life']['values']:
                    for orientation_item in self.cfg.default['settings']['orientation_array']:
                        df_temp = min_init_df[
                            (min_init_df.component == component_name) & (min_init_df.flaw_location == location_item) & (
                                    min_init_df.service_life == service_life) & (
                                    min_init_df.flaw_orientation == orientation_item)]
                        if len(df_temp) > 0:
                            df_temp = self.make_initial_flaw_monotonous(df_temp.copy())
                            plt_settings['label'] = [location_item.replace('_', ' ') + ', ' + orientation_item + ' orientation' + ', ' + str(
                                service_life) + ' years']
    ...
    def create_viz_min_init_flaw_by_service_life(self):
        ...
        for service_life in self.cfg.default['settings']['service_life']['values']:
            plt_settings = self.cfg['plot_settings'][2].copy()
            plt_settings['title'] = plt_settings['title'].format(str(service_life) + ' years')
            plt_settings.update({'file_name': self.cfg['Analysis']['result_folder'] + self.cfg['Analysis'][
                'file_name'] + '_' + plt_settings['file_name_extension'] + '_' + str(service_life) + '_years' + '.png'})
            for cfg_component_index in range(0, len(
                    self.cfg.loading['histograms']['from_xlsx'][0]['component_index'])):
                component_index = self.cfg.loading['histograms']['from_xlsx'][0]['component_index'][
                    cfg_component_index]
                component_name = self.histograms_df.columns[component_index]
                component_label = self.cfg.loading['histograms']['from_xlsx'][0]['component_label'][
                    cfg_component_index]
                ...
                for location_item in self.cfg.default['settings']['location_array']:
                    for orientation_item in self.cfg.default['settings']['orientation_array']:
                        ...
                        if len(df_temp) > 0:
                            df_temp = self.make_initial_flaw_monotonous(df_temp.copy())
                            plt_settings['label'] = [component_label + ', ' + location_item.replace('_',
    ...
    def create_viz_min_init_flaw_by_flaw_location(self):
        ...
        for location_item in self.cfg.default['settings']['location_array']:
            plt_settings = self.cfg['plot_settings'][2].copy()
            plt_settings['title'] = plt_settings['title'].format(location_item.replace('_', ' ') + ' flaw')
            plt_settings.update({'file_name': self.cfg['Analysis']['result_folder'] + self.cfg['Analysis'][
                'file_name'] + '_' + plt_settings['file_name_extension'] + '_' + location_item + '.png'})
            for cfg_component_index in range(0, len(
                    self.cfg.loading['histograms']['from_xlsx'][0]['component_index'])):
                component_index = self.cfg.loading['histograms']['from_xlsx'][0]['component_index'][
                    cfg_component_index]
                ...
                for service_life in self.cfg.default['settings']['service_life']['values']:
                    for orientation_item in self.cfg.default['settings']['orientation_array']:
                        ...
                        if len(df_temp) > 0:
                            df_temp = self.make_initial_flaw_monotonous(df_temp.copy())
                            plt_settings['label'] = [component_label + ', ' + orientation_item + ' orientation' + ', ' + str(
                                service_life) + ' years']
                            viz.from_df_columns(df_temp, plt_settings)
    ...
    def create_viz_min_init_flaw_by_flaw_orientation(self):
        ...
        for orientation_item in self.cfg.default['settings']['orientation_array']:
            plt_settings = self.cfg['plot_settings'][2].copy()
            plt_settings.update({'file_name': self.cfg['Analysis']['result_folder'] + self.cfg['Analysis'][
                'file_name'] + '_' + plt_settings['file_name_extension'] + '_' + orientation_item + '.png'})
            plt_settings['title'] = plt_settings['title'].format(orientation_item + ' Orientation')
            ...
            for cfg_component_index in range(0, len(
                ...
                for service_life in self.cfg.default['settings']['service_life']['values']:
                    for location_item in self.cfg.default['settings']['location_array']:
                        ...
                        if len(df_temp) > 0:
                            df_temp = self.make_initial_flaw_monotonous(df_temp.copy())
                            plt_settings['label'] = [component_label + ', ' + location_item.replace('_', ' ') + ', ' + str(service_life) + ' years']
                            viz.from_df_columns(df_temp, plt_settings)
        ...
        for cfg_component_index in range(0, len(self.cfg.loading['histograms']['from_xlsx'][0]['component_index'])):
            ...
            for location_item in self.cfg.default['settings']['location_array']:
                for orientation_item in self.cfg.default['settings']['orientation_array']:
                    ...
                    plt_settings.update({'file_name': self.cfg['Analysis']['result_folder'] + self.cfg['Analysis'][
                        'file_name'] + '_' + plt_settings['file_name_extension'] + '_component_' + str(
                        component_index) + '_' + location_item + '_' + orientation_item + '.png'})
                    ...
                    for flaw_index in range(0, len(c_array)):
                        if (flaw_index % 2 == 0):
                            plt_settings['label'] = ['flaw, start len = {} mm' .format(c_array[flaw_index])]
                            viz.from_df_columns(df_temp[df_temp.final_flaw_length == c_array[flaw_index]], plt_settings)
    ...
    def init_add_pipe_properties_to_cfg(self):
        ...
        self.cfg.pipe_properties = property_dictionary['section_properties']['pipe']
    ...
    def make_initial_flaw_monotonous(self, df, variable='final_flaw_length'):
        ...
        for row_index in range(0, len(df) -1):
            if df.final_flaw_depth[row_index+1] > df.final_flaw_depth[row_index]:
                df.ix[row_index+1, 'final_flaw_depth'] = df.final_flaw_depth[row_index]
        return df
