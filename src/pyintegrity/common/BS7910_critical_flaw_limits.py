class BS7910_2013():
    ...
    def get_critical_allowable_flaw(self):
        ...
        for location_item in self.cfg.default['settings']['location_array']:
            ...
            print("Performing Unstable Fracture Limit Analysis for flaw location: {} ..." .format(location_item))
            self.iterate_flaw_length_for_unstable_limits()
            logging.info(self.critical_allowable_flaw_df)
            self.result.update({location_item : {'fracture_limit': self.critical_allowable_flaw_df}})
            print("Unstable Fracture Limit Analysis for flaw location: {} ... COMPLETE \n" .format(location_item))
    ...
    def iterate_flaw_length_for_unstable_limits(self):
        ...
        for flaw_orientation in self.cfg.default['settings']['orientation_array']:
    ...
    def iterate_flaw_length_for_unstable_limits_for_single_c_value(self, a_nsteps, a_start, a_step, c):
        ...
        for flaw_theta in self.cfg.default['settings']['theta_array']:
            ...
            for a_index in range(0, a_nsteps):
                ...
                self.get_flaw_fad_properties()
                self.get_K_r_allowable()
                self.fad_properties_df.loc[len(self.fad_properties_df)] = [self.flaw.geometry,
                                                                           self.flaw.orientation,
                                                                           self.flaw.location,
                                                                           self.flaw.theta,
                                                                           a, c, self.K_r, self.L_r,
                                                                           self.K_r_allowable]
            for df_index in range(0, len(self.fad_properties_df) - 1):
                if self.fad_properties_df.K_r[df_index] > self.fad_properties_df.K_r_allowable[df_index]:
                    if df_index > 0:
            ...
            if self.cfg.default['settings']['save_detailed_results']:
                file_name = self.cfg['Analysis']['result_folder'] + self.cfg['Analysis'][
                    'file_name'] + '_' + 'len_{0}.csv'
                self.fad_properties_df.to_csv(file_name.format(c))

            self.critical_allowable_flaw_df.loc[len(self.critical_allowable_flaw_df)] = self.fad_properties_df.iloc[
                df_index - 1]
    ...
    def get_flaw_fad_properties(self):
        ...
        self.get_stress_intensity_factor_solution_by_annex(self.stress_intensity_annex)
        ...
        self.L_r = self.reference_stress/(self.material_grade_properties['SMYS'])
    ...
    def get_stress_intensity_factor_solution(self, stress_intensity_key):
        ...
        if ((self.flaw.location in ['external_surface', 'internal_surface']) and self.flaw.orientation in [
            'axial']) or (
                (self.flaw.location in ['internal_surface']) and self.flaw.orientation in ['circumferential']):
        ...
        elif (self.flaw.location in ['embedded']) or (
                self.flaw.location in ['external_surface'] and self.flaw.orientation in ['circumferential']):
            M_m = self.M_m
            M_b = self.M_b
            M_km = 1.0
        ...
        P_m = self.cfg.loading['primary_membrane_stress']['value'] * self.cfg.loading['primary_membrane_stress'][
            'membrane_factor'] * 0.00689476
        self.P_m = P_m
        P_b = self.cfg.loading['primary_membrane_stress']['value'] * self.cfg.loading['primary_membrane_stress'][
        ...
        Q_b = Q_m * self.cfg.loading['secondary_membrane_stress']['bending_factor']

        k_tm = self.k_tm
        k_tb = self.k_tb
        k_m = self.k_m

        self.Y_sigma_primary = M * fw * (k_tm * M_km * M_m * P_m + k_tb*M_kb*M_b*(P_b + (k_m -1) * P_m ))
        self.Y_sigma_secondary = M_m * Q_m + M_b * Q_b

        self.Y_delta_sigma_primary = M * fw * (k_tm * M_km * M_m * self.delta_P_m + k_tb * M_kb * M_b * (
    ...
    def get_finite_width_correction_factor(self):
        ...
        if self.stress_intensity_annex in ['Annex_M_7_2_2', 'Annex_M_7_2_4', 'Annex_M_7_3_2']:
            ...
            sec_of_value = math.sqrt(math.pi * self.c / self.W * math.sqrt(2 * self.a / self.B))
            try:
            ...
            sec_of_value = math.sqrt(math.pi * self.c / self.W * math.sqrt(2 * self.a / self.B_dash))
            try:
                fw = math.sqrt(1 / (math.cos(sec_of_value)))
            except:
                fw = 1
    ...
    def get_stress_intensity_factor_solution_by_annex(self, reference_section):
        reference_function = getattr(self, 'get_stress_intensity_factor_solution_' + reference_section)
        reference_function()
    ...
    def get_stress_intensity_factor_solution_Annex_M_4_1(self):
        ...
        if (self.a/(2*self.c)) <= 0.5:
            ...
            M_3 = 0.5 - (1 / (0.65 + self.a / self.c)) + 14 * (1 - self.a / self.c) ** 24
            g = 1 + (0.1 + 0.35 * (self.a / self.B) ** 2) * (1 - math.sin(self.theta)) ** 2
        else:
            M_1 = math.sqrt(self.c / self.a) * (1 + 0.04 * self.c / self.a)
            M_2 = 0.2 * (self.c / self.a) ** 4
            M_3 = -0.11 * (self.c / self.a) ** 4
            g = 1 + (0.1 + 0.35 * (self.c / self.a) * (self.a / self.B) ** 2) * (1 - math.sin(self.theta)) ** 2

        self.M_m = (M_1 + M_2 * (self.a / self.B) ** 2 + M_3 * (self.a / self.B) ** 4) * g * self.f_theta / self.phi

        if (self.a/(2*self.c)) <= 0.5:
            G_1 = -1.22 - 0.12 * self.a / self.c
            G_2 = 0.55 - 1.05 * (self.a / self.c) ** 0.75 + 0.47 * (self.a / self.c) ** 1.5
            q = 0.2 + self.a / self.c + 0.6 * (self.a / self.B)
            H_1 = 1 - 0.34 * (self.a / self.B) - 0.11 * (self.a / self.c) * (self.a / self.B)
        else:
            G_1 = -2.11 + 0.75 * self.c / self.a
            G_2 = 0.55 - 0.72 * (self.a / self.c) ** 0.75 + 0.14 * (self.a / self.c) ** 1.5
            q = 0.2 + self.c / self.a + 0.6 * (self.a / self.B)
            H_1 = 1 - (0.04 + 0.41 * (self.c / self.a) * (self.a / self.B)) + (
                    0.55 - 1.93 * (self.c / self.a) ** 0.75 + 1.38 * (self.c / self.a) ** 1.5) * (self.a / self.B) ** 2
    ...
    def get_stress_intensity_factor_solution_Annex_M_4_3(self):
        ...
        g = 1-( ((2*self.a/self.B_dash)**4) * (2.6-(4*self.a/self.B_dash)**0.5)/ (1 + 4*self.a/self.c) ) * math.fabs(math.cos(self.theta))
        self.M_m = (M_1 + M_2*(2*self.a/self.B_dash)**2 + M_3*(2*self.a/self.B_dash)**4)*g*self.f_theta/self.phi
        ...
        M_b_numerator = lamda_1 + lamda_2*(self.p/self.B) + lamda_3*(self.a/self.B) + lamda_4*(self.p*self.a/(self.B**2))
    ...
    def get_stress_intensity_factor_solution_Annex_M_7_2_2(self):
        ...
        if (not self.cfg.default['settings'].__contains__('stress_intensity_factor_solution')) or (
                self.cfg.default['settings']['stress_intensity_factor_solution'] == 'constant_values'):
            self.stress_intensity = {
                'deep':{'M_m': 0.932, 'M_b': 0.698, 'M_km': 1.0, 'M_kb': self.cfg.default['settings']['scf_bending']},
                'free_surface': {'M_m': 0.676, 'M_b': 0.632, 'M_km': 1.0, 'M_kb': self.cfg.default['settings']['scf_bending']}
            }
        else:
            self.stress_intensity = self.get_stress_intensity_solution_by_table_data(stress_intensity_solution_table)
    ...
    def get_stress_intensity_factor_solution_Annex_M_7_2_4(self):
        ...
        self.stress_intensity = {
            'deep':{'M_m': 0.953, 'M_b': 0.716, 'M_km': 1.0, 'M_kb': self.cfg.default['settings']['scf_bending']},
            'free_surface': {'M_m': 0.685, 'M_b': 0.641, 'M_km': 1.0, 'M_kb': self.cfg.default['settings']['scf_bending']}
        }
    ...
    def get_stress_intensity_factor_solution_Annex_M_7_3_2(self):
        ...
        self.stress_intensity = {
            'deep':{'M_m': 0.999, 'M_b': 0.731, 'M_km': 1.0, 'M_kb': self.cfg.default['settings']['scf_bending']},
            'free_surface': {'M_m': 0.731, 'M_b': 0.628, 'M_km': 1.0, 'M_kb': self.cfg.default['settings']['scf_bending']}
        }
    ...
    def get_stress_intensity_solution_by_table_data(self, stress_intensity_solution_table):
        ...
        if stress_intensity_solution_table in ['Table_M_2', 'Table_M_4', 'Table_M_7']:
            ...
            df = self.stress_intensity_solution_tables[stress_intensity_solution_table]
            theta = 90
            theta_location = 'Depth'
            M_m_deep, M_b_deep = self.dataframe_manual_interpolation_using_filters(B_over_ri, a_over_B, a_over_c, df, theta, theta_location)
            theta = 0
            theta_location = 'Surface'
            M_m_free_surface, M_b_free_surface = self.dataframe_manual_interpolation_using_filters(B_over_ri, a_over_B, a_over_c, df, theta, theta_location)

        self.stress_intensity = {
            'deep':{'M_m': M_m_deep, 'M_b': M_b_deep, 'M_km': 1.0, 'M_kb': self.cfg.default['settings']['scf_bending']},
            'free_surface': {'M_m': M_m_free_surface, 'M_b': M_b_free_surface, 'M_km': 1.0, 'M_kb': self.cfg.default['settings']['scf_bending']}
        }
    ...
    def dataframe_manual_interpolation_using_scipy(self, B_over_ri, a_over_B, a_over_c, df, theta, theta_location):
        ...
        X = [[a_over_B_list[index], a_over_c_list[index], B_over_ri_list[index]] for index in range(0, len(a_over_B_list))]
        Mm_list = df[df['theta'] == theta].Mm
        Mb_list = df[df['theta'] == theta].Mb
        n_neighbors = 8
        M_m = scipy_interp.solve_y_for_X(X, Mm_list, n_neighbors, [a_over_B, a_over_c, B_over_ri])
        M_b = scipy_interp.solve_y_for_X(X, Mb_list, n_neighbors, [a_over_B, a_over_c, B_over_ri])
        return M_m, M_b

    def dataframe_manual_interpolation_using_filters(self, B_over_ri, a_over_B, a_over_c, df, theta, theta_location):
        import statistics
        ...
        if a_over_B < a_over_B_look_up[0]:
            ...
            a_over_B_low = a_over_B_look_up[np.where(a_over_B_look_up <= a_over_B)][-1]
        if a_over_B > a_over_B_look_up[-1]:
            a_over_B_high = a_over_B_look_up[-1]
        else:
            a_over_B_high = a_over_B_look_up[np.where(a_over_B_look_up >= a_over_B)][0]

        if a_over_c < a_over_c_look_up[0]:
            a_over_c_low = a_over_c_look_up[0]
        else:
            a_over_c_low = a_over_c_look_up[np.where(a_over_c_look_up <= a_over_c)][-1]
        if a_over_c > a_over_c_look_up[-1]:
            a_over_c_high = a_over_c_look_up[-1]
        else:
            a_over_c_high = a_over_c_look_up[np.where(a_over_c_look_up >= a_over_c)][0]

        if B_over_ri < B_over_ri_look_up[0]:
            B_over_ri_low = B_over_ri_look_up[0]
        else:
            B_over_ri_low = B_over_ri_look_up[np.where(B_over_ri_look_up <= B_over_ri)][-1]
        if B_over_ri > B_over_ri_look_up[-1]:
            B_over_ri_high = B_over_ri_look_up[-1]
        else:
            B_over_ri_high = B_over_ri_look_up[np.where(B_over_ri_look_up >= B_over_ri)][0]

        df_temp_low = df[(df['a_over_B'] == a_over_B_low) & (df['a_over_c'] == a_over_c_low) & (
                df['B_over_ri'] == B_over_ri_low) & (df['theta'] == theta)].copy()
        df_temp_low.reset_index(inplace=True, drop=True)
        df_temp_high = df[(df['a_over_B'] == a_over_B_high) & (df['a_over_c'] == a_over_c_high) & (
                df['B_over_ri'] == B_over_ri_high) & (df['theta'] == theta)].copy()
        ...
        if a_over_B_low == a_over_B_high:
            ...
            a_over_B_factor = (df_temp['a_over_B'].iloc[1] - df_temp['a_over_B'].iloc[0]) / (
                    df_temp['a_over_B'].iloc[2] - df_temp['a_over_B'].iloc[0])

        if a_over_c_low == a_over_c_high:
            a_over_c_factor = 0
        else:
            a_over_c_factor = (df_temp['a_over_c'].iloc[1] - df_temp['a_over_c'].iloc[0]) / (
                df_temp['a_over_c'].iloc[2] - df_temp['a_over_c'].iloc[0])

        if B_over_ri_low == B_over_ri_high:
            B_over_ri_factor = 0
        else:
            B_over_ri_factor = (df_temp['B_over_ri'].iloc[1] - df_temp['B_over_ri'].iloc[0]) / (
                df_temp['B_over_ri'].iloc[2] - df_temp['B_over_ri'].iloc[0])

        average_factor = statistics.mean([a_over_B_factor, a_over_c_factor, B_over_ri_factor])

        M_m = df_temp['Mm'].iloc[0] + (df_temp['Mm'].iloc[2] - df_temp['Mm'].iloc[0]) * average_factor
        M_b = df_temp['Mb'].iloc[0] + (df_temp['Mb'].iloc[2] - df_temp['Mb'].iloc[0]) * average_factor
    ...
    def perform_flaw_dimension_acceptance_check(self):
        ...
        if a_over_B_flag:
            if (a_over_B >= a_over_B_range[0]) and (a_over_B <= a_over_B_range[1]):
                logging.info("a/B check acceptable")
            else:
                logging.info("flaw a/B, {} beyond code limits. Can not proceed further.".format(a_over_B))

        if a_over_c_flag:
            if (a_over_c >= a_over_c_range[0]) and (a_over_c <= a_over_c_range[0]):
                logging.info("a/c check acceptable")
            else:
                logging.info("flaw a/c, {} beyond code limits. Can not proceed further.".format(a_over_c))

        if B_over_ri_flag:
            if B_over_ri >= B_over_ri_range[0] and B_over_ri <= B_over_ri_range[1]:
                logging.info("B/ri check acceptable")
            else:
                logging.info("flaw B/ri, {} beyond code limits. Can not proceed further.".format(B_over_ri))
        if c_x_2_over_w_flag:
            if (c_x_2_over_w <= c_x_2_over_w_range[1]):
                logging.info("2c/W check acceptable")
            else:
                logging.info("flaw 2c/W, {} beyond code limits. Can not proceed further.".format(c_x_2_over_w))
        ...
        if a_over_B_dash_flag:
            if self.stress_intensity_annex in ['Annex_M_4_3']:
                if a_over_B_dash <= (0.625*(a_over_c) + 0.6):
                    ...
                    logging.info("flaw 2c/B_dash, {} beyond code limits. Can not proceed further.".format(a_over_B_dash))

    def get_reference_stress_by_annex(self, reference_section):
        reference_function = getattr(self, 'get_reference_stress_' + reference_section)
        reference_function()
    ...
    def get_reference_stress_by_equation(self, reference_equation):
        reference_function = getattr(self, 'get_reference_stress_' + reference_equation)
    ...
    def get_reference_stress_Equation_P_23(self):
        ...
        if (math.pi * self.radius) >= (self.c + self.B):
            ...
            alpha_double_dash = (self.a / self.B) * (self.c / (math.pi * self.radius))

        sigma_reference_term1 = self.P_m * (math.pi * (1 - self.a / self.B) + 2 * (self.a / self.B) * math.sin(self.c / self.radius)) / (
                    (1 - self.a / self.B) * (math.pi - (self.c / self.radius) * (self.a / self.B)))
    ...
    def get_reference_stress_Equation_P_11(self):
        ...
        if self.W >= 2*(self.c+self.B):
            ...
            self.alpha_double_dash = (2 * self.a / self.B) / (2 * self.c / self.W)

        sigma_reference_denominator = 3 * (
                    (1 - self.alpha_double_dash) ** 2 + 4 * self.p * self.alpha_double_dash / self.B)
        sigma_reference_numerator_term1 = self.P_b + 3 * self.P_m * self.alpha_double_dash
        sigma_reference_numerator_term2 = math.sqrt(sigma_reference_numerator_term1 ** 2 +
                                                    3 * self.P_m ** 2 * sigma_reference_denominator)

        sigma_reference = (sigma_reference_numerator_term1 +
    ...
    def init_assign_key_properties(self):
        ...
        self.material_properties['E'] = self.cfg['Material'][self.material]['E'] * 0.00689476
        self.material_grade_properties = self.cfg['Material'][self.material]['Grades'][self.material_grade].copy()
        self.material_grade_properties['SMYS'] = self.cfg['Material'][self.material]['Grades'][self.material_grade][
                                                     'SMYS'] * 0.00689476
        self.material_grade_properties['SMUS'] = self.cfg['Material'][self.material]['Grades'][self.material_grade][
                                                     'SMUS'] * 0.00689476
    ...
    #     TODO implement J-Integral method using section 7.1.4.6
        ratio_of_yield_to_tensile = self.material_grade_properties['SMYS']/self.material_grade_properties['SMUS']
        ...
        K_mat = math.sqrt(m * self.material_grade_properties['SMYS'] * delta_mat * self.material_properties['E'] / (
    ...
    def get_plasticity_correction(self):
        if (self.Y_sigma_secondary<0) or (self.Y_sigma_primary<=0):
            ...
            validation_ratio = self.Y_sigma_secondary/(self.Y_sigma_primary/self.L_r)
            if validation_ratio < 5.2:
                rho_1 = 0.1*(validation_ratio**0.714) - 0.007*(validation_ratio**2) + 0.00003*(validation_ratio**5)
            else:
                rho_1 = 0.25
            ...
            if self.L_r < 1.05:
                V_choice_1 = 1 + 0.2 * self.L_r + 0.02 * validation_ratio * (1 + 2 * self.L_r)
    ...
    def assign_key_flaw_dimensions_for_calculations(self):
        ...
        if self.flaw.location == 'external_surface':
            self.radius = self.cfg.pipe_properties['geometry']['Nominal_OD']/2*25.4
    ...
    def get_approximate_flaw_growth_due_to_histograms(self, histograms, bending_factor):
        ...
        service_life = max(self.cfg.default['settings']['service_life']['values'])
        for component_index in self.cfg.loading['histograms']['from_xlsx'][0]['component_index']:
            component_name = histograms.columns[component_index]
            print("Performing approximate initial flaw analysis for {0} ..." .format(component_name))

            if self.cfg.default['settings']['multi_process']:
                pool = Pool(number_of_workers)
                print("Performing Multi-process python using Pool workers {}" .format(number_of_workers))
                function_args = ((bending_factor, component_index, f_o_s, histograms, location_item, service_life, flaw_orientation)
                                 for location_item in self.cfg.default['settings']['location_array']
                                 for flaw_orientation in self.cfg.default['settings']['orientation_array'])
                flaw_growth_for_single_flaw_location_df_array = pool.starmap(self.get_approximate_flaw_growth_solution_for_single_flaw_location, function_args)
                pool.close()
                ...
                for location_item in self.cfg.default['settings']['location_array']:
                    for flaw_orientation in self.cfg.default['settings']['orientation_array']:
                        allowable_initial_flaw_for_single_flaw_location_df = self.get_approximate_flaw_growth_solution_for_single_flaw_location(bending_factor, component_index, f_o_s, histograms, location_item,
                                                                      service_life, flaw_orientation)
                        if not (len(allowable_initial_flaw_for_single_flaw_location_df) == 0):
                            self.approximate_flaw_growth_df = pd.concat([self.approximate_flaw_growth_df,
    ...
    def get_flaw_growth_due_to_histograms(self, histograms, bending_factor):
        ...
        self.flaw_growth_df = pd.DataFrame(
            columns=self.cfg.default['settings']['flaw_growth_properties_columns'])
        f_o_s = self.cfg.default['settings']['factor_of_safety']
        for component_index in self.cfg.loading['histograms']['from_xlsx'][0]['component_index']:
            component_name = histograms.columns[component_index]
            print("Performing time marching initial flaw analysis for {0} ..." .format(component_name))

            if self.cfg.default['settings']['multi_process']:
                pool = Pool(number_of_workers)
                print("Performing Multi-process python using Pool workers {}" .format(number_of_workers))
                function_args = ((bending_factor, component_index, f_o_s, histograms, location_item, flaw_orientation)
                                 for location_item in self.cfg.default['settings']['location_array']
                                 for flaw_orientation in self.cfg.default['settings']['orientation_array'])
                flaw_growth_for_single_flaw_location_df_array = pool.starmap(self.get_flaw_growth_for_single_flaw_location, function_args)
                pool.close()
                for df in flaw_growth_for_single_flaw_location_df_array:
                    if not (len(df) == 0):
                ...
                for location_item in self.cfg.default['settings']['location_array']:
                    for flaw_orientation in self.cfg.default['settings']['orientation_array']:
                        allowable_initial_flaw_for_single_flaw_location_df = self.get_flaw_growth_for_single_flaw_location(bending_factor, component_index, f_o_s, histograms, location_item,
                                                                      flaw_orientation)
                        if not (len(allowable_initial_flaw_for_single_flaw_location_df) == 0):
                            self.flaw_growth_df = pd.concat([self.flaw_growth_df,
                                                                         allowable_initial_flaw_for_single_flaw_location_df],
                                                                        sort=True)
    ...
    def get_flaw_growth_for_single_flaw_location(self, bending_factor, component_index, f_o_s, histograms,
        ...
        self.flaw.orientation = flaw_orientation
        component_name = histograms.columns[component_index]
        print("            ..... for component flaw location: {1}, flaw orientation: {2} ...".format(component_name, location_item, flaw_orientation))
        for c_index in range(0, len(self.cfg.default['settings']['c_array'])):
            self.flaw.dimensions['c'] = self.cfg.default['settings']['c_array'][c_index]
            df_temp = self.approximate_flaw_growth_df[(self.approximate_flaw_growth_df.component == component_name) & (self.approximate_flaw_growth_df.flaw_location == location_item) & (self.approximate_flaw_growth_df.flaw_orientation == flaw_orientation) & (self.approximate_flaw_growth_df.initial_flaw_length ==self.flaw.dimensions['c'])].copy()
            try:
                if (len(df_temp) > 0)  and (df_temp.initial_flaw_depth.iloc[0] is not None):
                    self.flaw.dimensions['a'] = round(df_temp.initial_flaw_depth.iloc[0]* 0.75, 1)
                else:
                    self.flaw.dimensions['a'] = 0.8
            except:
                self.flaw.dimensions['a'] = 0.8
            service_life = 0
            solution_end_life = max(self.cfg.default['settings']['service_life']['values'])
            solution_step = self.cfg.default['settings']['service_life']['solution_step']
            life_nsteps = int((solution_end_life * 4) / solution_step)
            self.get_flaw_fad_properties()
            self.get_K_r_allowable()
            self.flaw_growth_for_single_flaw_location_df.loc[len(self.flaw_growth_for_single_flaw_location_df)] = [
                ...
                self.flaw.dimensions['c'],
                self.K_r, self.L_r,
                self.K_r_allowable]

            for life_index in range(0, life_nsteps):
                if service_life >= 2*solution_end_life:
                    solution_step = 4*self.cfg.default['settings']['service_life']['solution_step']
                elif service_life >= solution_end_life:
                    solution_step = 2*self.cfg.default['settings']['service_life']['solution_step']
                else:
                    solution_step = self.cfg.default['settings']['service_life']['solution_step']
                ...
                for histogram_row_index in range(0, len(histograms) - 1):
                    number_of_cycles = (histograms.iloc[histogram_row_index][
                                            histograms.columns[component_index]] / 2) * solution_step * f_o_s
                    if number_of_cycles > 0:
                        self.delta_P_m = 0
                        self.delta_P_b = (histograms.iloc[histogram_row_index]['bins'] +
                                          histograms.iloc[histogram_row_index + 1][
                                              'bins']) / 2 / 1000 * bending_factor
                        self.get_flaw_growth_from_one_cycle()

                        delta_a_per_bin = self.delta_a_per_cycle_stress * number_of_cycles
                        self.flaw.dimensions['a'] = self.flaw.dimensions['a'] + delta_a_per_bin
                ...
                if self.K_r >= self.K_r_allowable:
                    ...
                    self.flaw_growth_for_single_flaw_location_df.loc[
                        len(self.flaw_growth_for_single_flaw_location_df)] = [
                        histograms.columns[component_index],
                        service_life,
                        self.flaw.geometry,
                        self.flaw.orientation,
        ...
        print("            ..... for component flaw location: {1}, flaw orientation: {2} ... COMPLETE".format(component_name, location_item, flaw_orientation))
    ...
    def get_flaw_growth_from_one_cycle(self):
        ...
        if self.delta_K_I <= self.cfg.paris_curve['transfer_delta_K']:
            ...
            self.delta_a_per_cycle_stress = self.cfg.paris_curve['A_2']*(self.delta_K_I)**self.cfg.paris_curve['m_2']

        logging.info("crack size_growth_ per cycle for stress range {0}  MPa is : {1} mm".format(self.delta_P_b,
    ...
    def get_initial_allowable_flaw_for_life(self, histograms):
        ...
        for cfg_component_index in range(0, len(self.cfg.loading['histograms']['from_xlsx'][0]['component_index'])):
            component_index = self.cfg.loading['histograms']['from_xlsx'][0]['component_index'][cfg_component_index]
            component_name = histograms.columns[component_index]
            for location_item in self.cfg.default['settings']['location_array']:
                for orientation_item in self.cfg.default['settings']['orientation_array']:
                    ...
                    for flaw_length in c_array:
                        df_temp = self.flaw_growth_df[
                            (self.flaw_growth_df.component == component_name) & (
                                    ...
                                    self.flaw_growth_df.flaw_orientation == orientation_item) & (
                                    self.flaw_growth_df.final_flaw_length == flaw_length)]
                        for service_life in self.cfg.default['settings']['service_life']['values']:
                            if df_temp.service_life.max() >= service_life:
                                df_temp_service_life = df_temp_service_life = df_temp[df_temp.service_life <= (df_temp.service_life.max() - service_life)].tail(1).copy()
                                df_temp_service_life.service_life = service_life
                                self.initial_allowable_flaw_df = pd.concat([self.initial_allowable_flaw_df,
                                                             df_temp_service_life], sort=True)
    ...
    def get_approximate_flaw_growth_solution_for_single_flaw_location(self, bending_factor, component_index, f_o_s, histograms,
        ...
        for c in self.cfg.default['settings']['c_array']:
            ...
            for a_index in range(0, a_nsteps):
                ...
                for histogram_row_index in range(0, len(histograms) - 1):
                    number_of_cycles = (histograms.iloc[histogram_row_index][
                                            histograms.columns[component_index]] / 2) * service_life * f_o_s
                    if number_of_cycles > 0:
                        self.delta_P_m = 0
                        self.delta_P_b = (histograms.iloc[histogram_row_index]['bins'] +
                                          histograms.iloc[histogram_row_index + 1][
                ...
                self.get_K_r_allowable()
                self.approximate_flaw_growth_for_single_flaw_location_and_length_df.loc[len(self.approximate_flaw_growth_for_single_flaw_location_and_length_df)] = [
                    histograms.columns[component_index],
                    service_life,
                    self.flaw.geometry,
                    self.flaw.orientation,
            ...
            for df_index in range(0, len(self.approximate_flaw_growth_for_single_flaw_location_and_length_df) - 1):
                if self.approximate_flaw_growth_for_single_flaw_location_and_length_df.K_r[df_index] > self.approximate_flaw_growth_for_single_flaw_location_and_length_df.K_r_allowable[df_index]:
                    a_end = self.approximate_flaw_growth_for_single_flaw_location_and_length_df.initial_flaw_depth[df_index]
                    # a_nsteps = int((a_end - a_start) / a_step + 1)
                    break

            if self.cfg.default['settings']['save_detailed_results']:
                self.approximate_flaw_growth_for_single_flaw_location_and_length_df.to_csv('results/len_{}.csv'.format(c))
            ...
            try:
                ...
                print("IndexError: single positional indexer is out-of-bounds for df_index: {}".format(df_index))
